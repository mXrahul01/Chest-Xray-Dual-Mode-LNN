...
model1 = LiquidNN(224*224, 128, 2).to(DEVICE)
model2 = LiquidNN(224*224, 128, 2).to(DEVICE)
...
LABELS_1 = ['Normal', 'Pneumonia']
LABELS_2 = ['Normal', 'Lung Opacity']
...
def overlay_heatmap_on_pil(pil_img: Image.Image, heatmap: np.ndarray, alpha: float = 0.4) -> Image.Image:
    ...
    heat_rgba = (color_map(heatmap) * 255).astype(np.uint8)
...
@app.route("/")
def index():
    # === cleanup logic for single mode ===
    # Clean uploads and heatmaps folder when going back to homepage
    for folder in [UPLOAD_DIR, HEATMAP_DIR]:
...
def upload_single():
    ...
    try:
        ...
        pass  # Do not cleanup here, files needed for result page display
